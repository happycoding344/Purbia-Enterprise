...
class LrController extends Controller
{
    ...
    public function update(Request $request, Lr $lr)
    {
        $request->validate([
            'financial_year' => 'required|string',
            'lr_date' => 'required|date',
            'location_from_id' => 'required|exists:locations,id',
            'lr_no' => 'required|string|unique:lrs,lr_no,' . $lr->id,
            'to_city_id' => 'required|exists:cities,id',
            'from_city_id' => 'required|exists:cities,id',
            'state_id' => 'required|exists:states,id',
            'delivery_place_id' => 'required|exists:delivery_places,id',
        ...
        try {
            return DB::transaction(function () use ($request, $lr) {
                $data = $request->except(['items', 'attachments', '_method']);
                ...
                // Log activity
                ActivityLog::log('updated', 'LR', $lr->id, "Updated LR #{$lr->lr_no}", [
                    'lr_no' => $lr->lr_no,
                    'total_amount' => $lr->total_amount,
                ]);

                return response()->json($lr->load('items', 'attachments'));
            });
        } catch (\Exception $e) {
            return response()->json([
